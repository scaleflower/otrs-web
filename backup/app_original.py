...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///../db/otrs_data.db'
...
class DailyStatistics(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    statistic_date = db.Column(db.Date, nullable=False, unique=True)
...
def calculate_age_distribution():
    ...
    try:
        ...
        print(f"Error calculating daily statistics: {str(e)}")
        # Log error
        try:
            with app.app_context():
                log_entry = StatisticsLog(
...
def schedule_age_distribution(scheduler):
    ...
    try:
        ...
        try:
            scheduler.remove_job('age_distribution_job')
        except:
            pass
        
        # Add new job with configured time
        scheduler.add_job(
            func=calculate_age_distribution,
...
def export_txt():
    ...
    try:
        ...
        return jsonify({'error': f'Error exporting text file: {str(e)}'}), 500
...
def api_responsible_stats():
    ...
    try:
        data = request.get_json()
        if not data or 'selected_responsibles' not in data:
            return jsonify({'error': 'Missing required data'}), 400
        
        selected_responsibles = data['selected_responsibles']
        
        if not selected_responsibles:
            return jsonify({'error': 'No responsible persons selected'}), 400
        
        # Get statistics for selected responsibles
        stats = {}
        ...
        if existing_config:
            existing_config.selected_responsibles = str(selected_responsibles)
            existing_config.updated_at = datetime.utcnow()
        else:
            new_config = ResponsibleConfig(
                user_identifier=user_ip,
                selected_responsibles=str(selected_responsibles)
            )
        ...
        return jsonify({'error': f'Error getting responsible statistics: {str(e)}'}), 500

@app.route('/api/responsible-list')
def api_responsible_list():
    """Get list of all responsible persons"""
    try:
        responsibles = db.session.query(OtrsTicket.responsible).filter(
        ...
        return jsonify({'error': f'Error getting responsible list: {str(e)}'}), 500
...
def api_daily_statistics():
    ...
    try:
        ...
        data = {
            'daily_stats': [
                {
                    ...
                    'closing_balance': stat.closing_balance,
                    'age_lt_24h': stat.age_lt_24h,
                    'age_24_48h': stat.age_24_48h,
                    'age_48_72h': stat.age_48_72h,
                    'age_72_96h': stat.age_72_96h,
                    'age_gt_96h': stat.age_gt_96h
                }
            ...
            'stats_logs': [
                {
                    ...
                    'statistic_date': str(log.statistic_date) if log.statistic_date else None,
                    'age_24h': log.age_24h,
                    'age_24_48h': log.age_24_48h,
                    'age_48_72h': log.age_48_72h,
                    'age_72_96h': log.age_72_96h,
                    'total_open': log.total_open,
                    'status': log.status,
            ...
            'config': {
                'schedule_time': config.schedule_time if config else '23:59',
                'enabled': config.enabled if config else True
            } if config else {'schedule_time': '23:59', 'enabled': True}
        }
        ...
        return jsonify({'error': f'Error getting daily statistics: {str(e)}'}), 500
...
def api_update_schedule():
    ...
    try:
        ...
        # Reschedule the job - we can't access the scheduler from here in the API context
        # The job will be rescheduled when the application restarts
        print(f"Schedule updated to {schedule_time}, enabled: {enabled}. Changes will take effect after application restart.")
...
def api_calculate_daily_stats():
    ...
    try:
        ...
        return jsonify({'error': f'Error calculating daily statistics: {str(e)}'}), 500
...
def api_export_execution_logs():
    ...
    try:
        ...
        output.seek(0)
        filename = f"execution_logs_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        ...
        return jsonify({'error': f'Error exporting execution logs: {str(e)}'}), 500
...
def clear_database():
    ...
    try:
        ...
        if ticket_count == 0:
            return jsonify({
                'success': True,
                'message': '数据库已经是空的，无需清除',
                'records_cleared': 0
            })

        # Get record count before deletion for logging
        records_cleared = ticket_count
        ...
        log_database_operation(
            operation_type='clear_tickets',
            table_name='otrs_ticket',
            records_affected=records_cleared,
        ...
        return jsonify({
            'success': True,
            'message': f'成功清除 {records_cleared} 条工单记录',
...
if __name__ == '__main__':
    ...
    schedule_age_distribution(scheduler)
    scheduler.start()
